...
void
show_classes_to_nanny (DESCRIPTOR_DATA * d)
{
  CHAR_DATA *ch;
  sh_int cnt = 0;
  int iClass;
...
void
show_races_to_nanny (DESCRIPTOR_DATA * d)
{
  CHAR_DATA *ch;
  sh_int cnt = 0;
  int iRace;
...
/*	extern int lang_array[];
  ...
  switch (d->connected)
      ...
      if (!str_cmp (arg, _("help")))
	{
	  ...
	  ch_printf_color (ch,
			   "&R* I'm afraid there is no description for that class.\n\r\n\r");
	  show_classes_to_nanny (d);
	  return;
	}
	  ...
	  || !str_cmp (class_table[iClass]->who_name, "unused"))
	{
	  if (arg[0] != '\0')
	    ch_printf_color (ch,
			     "&R* I'm afraid there is no such class available.\n\r\n\r");
	  show_classes_to_nanny (d);
	  return;
	}
      ...
      if (!str_cmp (arg, _("help")))
	{
	  ...
	  ch_printf_color (ch,
			   "&R* I'm afraid there is no description for that race.\n\r\n\r");
	  show_races_to_nanny (d);
	  return;
	}
	  ...
	  || !str_cmp (race_table[iRace]->race_name, "unused"))
	{
	  if (arg[0] != '\0')
	    ch_printf_color (ch,
			     "&R* I'm afraid there is no such race available.\n\r\n\r");
	  show_races_to_nanny (d);
	  return;
	}
...
*/
      ...
      if (ch->level == 0)
	{
	  ...
	  ch->height =
	    number_range (race_table[ch->race]->height * .9,
			  race_table[ch->race]->height * 1.1);
	  ch->weight =
	    ...
	    ch->pcdata->release_date > current_time)
	{
	  if (ch->in_room->vnum == 6
	      || ch->in_room->vnum == 8 || ch->in_room->vnum == 1206)
	...
	if (ch->in_room && (IS_IMMORTAL (ch)
			    ||
			    (!xIS_SET
			     (ch->in_room->room_flags, ROOM_PROTOTYPE)
  ...
  for (ch = first_char; ch; ch = ch->next)
    {
      if (!IS_NPC (ch)
	  && (!fConn || !ch->desc)
	  && ch->pcdata->filename && !str_cmp (name, ch->pcdata->filename))
	{
...
void
act (sh_int AType, const char *format, CHAR_DATA * ch, const void *arg1,
...
#define ACTF_OBJ  BV02
  ...
  if (flags1 != ACTF_NONE && flags1 != ACTF_TXT && flags1 != ACTF_CH
    ...
    {
      bug
	("Act: arg1 has more than one type in format %s.  Setting all NULL.",
	 format);
      obj1 = NULL;
    }
  ...
  if (flags2 != ACTF_NONE && flags2 != ACTF_TXT && flags2 != ACTF_CH
    ...
    {
      bug
	("Act: arg2 has more than one type in format %s.  Setting all NULL.",
	 format);
      vch = NULL;
      obj2 = NULL;
    }
...
void
do_name (CHAR_DATA * ch, char *argument)
{
  char fname[1024];
  struct stat fst;
  CHAR_DATA *tmp;
 ...
 */
int
colorlen (const char *s, int goal)
{
  const char *p = s;
	  ...
	  *pbuf = '\0';
      ...
      case 'd':
	    ...
	    case 'p':		/* display who is on appear list. */
	      if (IS_IMMORTAL (ch))
		sprintf (pbuf, "(%s)",
			 ch->pcdata->see_me[0] ? ch->pcdata->see_me : "");
	      break;
	    case 'R':
...
int
make_color_sequence (const char *col, char *buf, DESCRIPTOR_DATA * d)
{
  int ln = 0;
...
pager_output (DESCRIPTOR_DATA * d)
{
  ...
  if (xIS_SET (ch->act, PLR_ANSI))
	...
	sprintf (buf, "\033[0;%s%s3%dm",
		 d->prevcolor & 8 ? "1;" : "",
		 d->prevcolor > 15 ? "5;" : "", d->prevcolor & 7);
